...
/*   Updated: 2023/07/28 23:03:44 by gbarone          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	g_render(t_game *game, t_vector *ghost)
{
	...
	u = game->total_max_fr / 4;
	x = ghost->x * 50;
	y = ghost->y * 50;
	if (current_frame <= u)
		mlx_put_image_to_window(game->mlx, game->w, game->imgs->g1, x, y);
	else if (current_frame <= u * 2)
		mlx_put_image_to_window(game->mlx, game->w, game->imgs->g2, x, y);
	else if (current_frame <= u * 3)
		mlx_put_image_to_window(game->mlx, game->w, game->imgs->g3, x, y);
	else
		mlx_put_image_to_window(game->mlx, game->w, game->imgs->g4, x, y);
}
